...
Rich Resume TXT -> Normalized JSON converter
- General-purpose (AI/ML, Fullstack, Testing, DB modeling, Cloud, Solution Arch, Sales, etc.)
- Uses Groq function-calling for consistent structured output.
- No retries, no fallback parsing: if the model does not return the function-call JSON, the file is skipped and logged.
"""
...
OUTPUT_DIR = "ProcessedJson"
LOG_FILE = "processing_errors.log1"
MODEL_NAME = "llama-3.1-70b-versatile"   # Free Groq model that supports reliable tool calling
MAX_RESPONSE_TOKENS = 2500
# ---------------------------
...
# Safe initialization so the script doesn't crash on import
client = OpenAI(
...
def load_domain_tags():
    try:
        ...
        logging.warning(f"Could not load domain tags from {JD_JSON}: {e}")
        return []
...
def process_resume_file(path: str) -> Dict[str, Any]:
    ...
    resp = client.chat.completions.create(
        ...
        tool_choice={"type": "function", "function": {"name": "parse_resume_detailed"}},
        temperature=0.0,
        max_tokens=MAX_RESPONSE_TOKENS
    )
    ...
    if getattr(msg, "tool_calls", None) is None or len(msg.tool_calls) == 0:
        raise RuntimeError("Model did not return a tool_call response. Skipping file.")
...
def write_example(output_dir: str):
    out_path = Path(output_dir) / "example_output.json"
    print(f"[INFO] Example JSON logic bypassed.")
